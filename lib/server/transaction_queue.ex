defmodule Kylix.Server.TransactionQueue do
  ...
  * `interval_ms` - New processing interval in milliseconds
    ...
    Enum.reduce(1..batch_size, {state, 0}, fn i, {current_state, count} ->
      ...
      case :queue.out(current_state.queue) do
        {{:value, tx_data}, new_queue} ->
          ...
          result = execute_transaction(tx_data)

          now = DateTime.utc_now()

          updated_statuses =
            update_transaction_status(current_state.transaction_statuses, ref, result, now)

          new_stats = update_stats(current_state.stats, result, now)
  ...
  defp execute_transaction(tx_data) do
    try do
      Kylix.BlockchainServer.add_transaction(
        tx_data.subject,
        tx_data.predicate,
      ...
      e ->
        Logger.error("Error processing transaction #{inspect(tx_data.ref)}: #{inspect(e)}")
  ...
  defp update_transaction_status(transaction_statuses, ref, result, now) do
    Map.put(transaction_statuses, ref, %{
      result: result,
      completed_at: now
    })
  end

  defp update_stats(stats, result, now) do
    case result do
      {:ok, _tx_id} ->
        %{stats | processed: stats.processed + 1, last_processed_at: now}
