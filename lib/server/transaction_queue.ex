defmodule Kylix.Server.TransactionQueue do
  ...
  * `interval_ms` - New processing interval in milliseconds
  ...
  defp find_and_remove_from_queue(queue, ref) do
    do_find_and_remove_from_queue(queue, ref, :queue.new())
  end

  defp do_find_and_remove_from_queue(queue, ref, acc) do
    case :queue.out(queue) do
      {{:value, tx}, rest} ->
        if tx.ref == ref do
          {tx, :queue.join(acc, rest)}
        else
          do_find_and_remove_from_queue(rest, ref, :queue.in(tx, acc))
        end
